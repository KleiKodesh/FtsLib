...
class _MainScreenState extends State<MainScreen> {
    ...
    try {
      await _indexService.build(
        ...
        onProgress: (indexed, total) {
          if (!mounted) return;
          print('MainScreen: Progress callback received: $indexed/$total');
          setState(() {
  ...
  void _updateEta() {
    ...
    if (pct >= 1.0) {
      ...
      final remainSec =
          (totalSec - elapsed.inSeconds).clamp(0, double.infinity);
      remaining = Duration(seconds: remainSec.toInt());
    } else {
      const baselineSec = baselineMinutes * 60;
      remaining = Duration(
          seconds:
              (baselineSec - elapsed.inSeconds).clamp(0, baselineSec).toInt());
    }
